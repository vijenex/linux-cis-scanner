...
func CheckPAMConfig(filePath, module, parameter, expected, description string) CheckResult {
	...
	// Handle "not_found" expected value
	if expected == "not_found" {
		if !found {
			return CheckResult{
				Status:          StatusPass,
				ActualValue:     "parameter not found (as expected)",
			...
			return CheckResult{
				Status:          StatusFail,
				ActualValue:     fmt.Sprintf("parameter found: %s", actual),
				EvidenceCommand: fmt.Sprintf("grep %s %s", module, filePath),
				Description:     description,
			}
		}
	...
	status := StatusFail
	// For "enabled" expected value, just check if parameter exists (found = true)
	if expected == "enabled" {
		if found {
			status = StatusPass
		}
