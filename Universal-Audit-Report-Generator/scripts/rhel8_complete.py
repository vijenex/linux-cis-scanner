#!/usr/bin/env python3
"""RHEL-8 CIS Audit Report - Complete Windows Style with All Controls"""
import os, re, csv
        ...
        for row in parse_csv(csv_path):
            ...
            if control_id not in control_results:
                control_results[control_id] = {
                    ...
                    'section': control_id.split('.')[0] if '.' in control_id else '1',
                    'passed_prod': set(),
                    'failed_prod': set(),
                    'passed_nonprod': set(),
                    'failed_nonprod': set()
...
print(f"Found {len(control_results)} controls")
...
for control_id in control_ids:
    ...
    p.add_run(f"This control ensures {data['title'].lower()}. Refer to CIS Red Hat Enterprise Linux 8 Benchmark v4.0.0 for complete details.")
    doc.add_paragraph()
    
    p = doc.add_paragraph()
    p.add_run('Impact: ').bold = True
    p.add_run('Implementing this control enhances system security posture. Review the official CIS Benchmark for specific impact details.')
    doc.add_paragraph()
    
    p = doc.add_paragraph()
    p.add_run('Remediation: ').bold = True
    p.add_run('Follow the remediation steps outlined in the CIS Red Hat Enterprise Linux 8 Benchmark v4.0.0 for this control.')
    doc.add_paragraph()
